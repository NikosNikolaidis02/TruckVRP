...
class Solver:
    ...
    def CalculateTotalCost(self, sol):
        ...
        for i in range(0, len(sol.routes)):
            ...
            tot_time = 0
            # in range(0, len(rt.sequenceOfNodes) - 2) in order to not calculate the cost of returning to the depot
            for j in range(0, len(rt.sequenceOfNodes) - 2):
                a = rt.sequenceOfNodes[j]
                b = rt.sequenceOfNodes[j+1]
                tot_time += self.distanceMatrix[a.ID][b.ID]
                rt_cumulative_cost += tot_time
                tot_time += b.unloading_time
